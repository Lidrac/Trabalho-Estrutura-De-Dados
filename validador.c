...
//remove aspas simples e duplas e espaços extras
...
//verifica se o comando bate com a palavra
...
//busca uma palavra chave no comando ignorando maiusculas e minusculas
char* encontrarInicioKeyword(char *linha, char *keyword) {
    char linhaLower[300];
    char keywordLower[50];

    if (strlen(linha) >= 300 || strlen(keyword) >= 50) return NULL;

    // Copia linha para minusculo
    strcpy(linhaLower, linha);
    for(int i=0; linhaLower[i]; i++) linhaLower[i] = tolower(linhaLower[i]);

    // Copia keyword para minusculo
    strcpy(keywordLower, keyword);
    for(int i=0; keywordLower[i]; i++) keywordLower[i] = tolower(keywordLower[i]);

    // Busca na versão minuscula
    char *ptrLower = strstr(linhaLower, keywordLower);
    
    if(!ptrLower) return NULL;

    // Calcula a distancia e retorna o ponteiro na linha original
    int offset = ptrLower - linhaLower;
    return linha + offset;
}
// converte data texto para inteiro
...
int separar_colunas_valores (char *linha, char *bufferColumns, char *bufferValues){
    ...
    char *ptrValues = encontrarInicioKeyword(linha, "values");
    if(!ptrValues) return 0;

    int offsetValues = ptrValues - linha;

    // achar parênteses das colunas (antes do valores)
    char *EndColumns = linha + offsetValues;
    char *StartColumns = strchr(linha, '(');
    ...
    char *StartValues = strchr(ptrValues, '(');
...
//quebra o comando em partes separadas por vírgula, so que ignora virgula dentro de aspas
...
int buscar_valor(char *linha, char *nomeCampo, char *destino) {
    ...
    char *ptrCampo = encontrarInicioKeyword(linha, nomeCampo);
...
//funções padrão para adicionar nas filas
...
//se der erro, essa função limpa toda memoria alocada
...
//processa o comando e preenche as estruturas de acordo
int processar_tipo_pet(char *linha, int operacao, NoFilaTipoPet **fila) {
    ...
    if (operacao == 1) { // INSERT DINÂMICO
        ...
        // Lógica do UPDATE
        if(operacao == 3) {
            char *ptrSet = encontrarInicioKeyword(linha, "set");  
        ...
        // Lógica do WHERE
        char *ptrWhere = encontrarInicioKeyword(linha, "where");
        ...
        // Lógica ORDER BY
        if(operacao == 4) {
            if(encontrarInicioKeyword(linha, "descricao") || encontrarInicioKeyword(linha, "nome")) strcpy(cmd.criterioOrdenacao, "nome");
            else if(encontrarInicioKeyword(linha, "codigo")) strcpy(cmd.criterioOrdenacao, "codigo");
        }
    }
    ...
    if (operacao == 1) { // INSERT DINÂMICO
        ...
        if(operacao == 3) {
            ...
            if(ptrSet) {
                ...
                if(buscar_valor(ptrSet, "fone", buffer) || buscar_valor(ptrSet, "telefone", buffer)) strcpy(cmd.dadosPessoa->fone, buffer);
                if(buscar_valor(ptrSet, "endereco", buffer)) strcpy(cmd.dadosPessoa->endereco, buffer);
                if(buscar_valor(ptrSet, "data_nascimento", buffer) || buscar_valor(ptrSet, "data", buffer)) strcpy(cmd.dadosPessoa->dataNascimento, buffer);
            }
        }
        ...
        if(ptrWhere) {
            if(buscar_valor(ptrWhere, "codigo", buffer)) {
                ...
                strcpy(cmd.tipoFiltragem, "nome");
            } else if(buscar_valor(ptrWhere, "fone", buffer) || buscar_valor(ptrWhere, "telefone", buffer)) {
            ...
            } else if (buscar_valor(ptrWhere, "data_nascimento", buffer) || buscar_valor(ptrWhere, "data", buffer)) {
        ...
        // ORDER
        if(operacao == 4) {
             if(encontrarInicioKeyword(linha, "nome")) strcpy(cmd.criterioOrdenacao, "nome");
             else if(encontrarInicioKeyword(linha, "data")) strcpy(cmd.criterioOrdenacao, "data");
             else if(encontrarInicioKeyword(linha, "fone") || encontrarInicioKeyword(linha, "telefone")) strcpy(cmd.criterioOrdenacao, "fone");
             else if(encontrarInicioKeyword(linha, "codigo")) strcpy(cmd.criterioOrdenacao, "codigo");
             else if(encontrarInicioKeyword(linha, "data") || encontrarInicioKeyword(linha, "data_nascimento")) strcpy(cmd.criterioOrdenacao, "data");
    ...
    if (operacao == 1) { // INSERT DINÂMICO
        ...
        if(operacao == 3) {
            ...
            if(ptrSet) {
                ...
                if(buscar_valor(ptrSet, "codigo_pes", buffer)  || buscar_valor(ptrSet, "dono", buffer) || buscar_valor(ptrSet, "codigo_cli", buffer)) cmd.dadosPet->codigo_pes = atoi(buffer);
                if(buscar_valor(ptrSet, "codigo_tipo", buffer)|| buscar_valor(ptrSet, "tipo", buffer)) cmd.dadosPet->codigo_tipo = atoi(buffer);
            }
        }
        ...
        char *ptrWhere = encontrarInicioKeyword(linha, "where");
        if(ptrWhere) {
            if(buscar_valor(ptrWhere, "codigo_pes", buffer) || buscar_valor(ptrWhere, "dono", buffer) || buscar_valor(ptrWhere, "codigo_cli", buffer)) {
                cmd.dadosPet->codigo_pes = atoi_limpo(buffer);
                strcpy(cmd.tipoFiltragem, "codigo_pes");
            } else if(buscar_valor(ptrWhere, "codigo_tipo", buffer) || buscar_valor(ptrWhere, "tipo", buffer)) {
        ...
        if(operacao == 4) {
            if(encontrarInicioKeyword(linha, "nome")) strcpy(cmd.criterioOrdenacao, "nome");
            else if(encontrarInicioKeyword(linha, "codigo_pes") || encontrarInicioKeyword(linha, "codigo_cli") || encontrarInicioKeyword(linha, "dono")) strcpy(cmd.criterioOrdenacao, "codigo_pes");
            else if(encontrarInicioKeyword(linha, "codigo_tipo") || encontrarInicioKeyword(linha, "tipo")) strcpy(cmd.criterioOrdenacao, "codigo_tipo");
            else if(encontrarInicioKeyword(linha, "codigo")) strcpy(cmd.criterioOrdenacao, "codigo");
        }
    }
...
//distribrui os comandos brutos para as filas específicas
int DistribuirComandos(Fila *FilaBruta, NoFilaPessoa **FilaPessoa, NoFilaPet **FilaPet, NoFilaTipoPet **FilaTipo) {
    ...
    while (atual) {
        ...
        //se a sintaxe estiver zoada, limpa tudo
        if (sucesso == 0) {
