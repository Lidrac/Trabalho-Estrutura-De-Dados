...
void alterarPessoaPorCriterio(NoPessoa **iniPes, NoPet **iniPet, comandoPessoa cmd) {
    ...
    while (atual != NULL) {
        ...
        if (deveAlterar) {
            cmd.dadosPessoa->codigo = atual->p->codigo;
...
void executarSelectPessoa(NoPessoa *lista, comandoPessoa cmd) {
    ...
    while (aux) {
        if (aux->p->ativo) {
            ...
            else if (strcasecmp(cmd.tipoFiltragem, "data") == 0 && strstr(aux->p->dataNascimento, cmd.dadosPessoa->dataNascimento)) match = 1;
...
void alterarPetPorCriterio(NoPet **iniPet, NoPessoa **iniPes, NoTipoDePet **iniTipoDePet, comandoPet cmd) {
    ...
    while (atual != NULL) {
        ...
        if (deveAlterar) {
            cmd.dadosPet->codigo = atual->p->codigo;
...
void alterarTipoDePetPorCriterio(NoTipoDePet **iniTipoDePet, NoPet **iniPet, comandoTipoPet cmd) {
    ...
    while (atual != NULL) {
        ...
        if (deveAlterar) {
            cmd.dadosTipoDePet->codigo = atual->p->codigo;
